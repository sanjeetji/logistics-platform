...
public class BillingService {
    ...
    public List<Invoice> getPendingInvoices(String clientId) {
        return invoiceRepository.findByClientId(clientId).stream()
                .filter(inv -> inv.getStatus() == Invoice.InvoiceStatus.ISSUED || inv.getStatus() == Invoice.InvoiceStatus.OVERDUE)
    ...
    public BillingDtos.LedgerResponse getClientLedger(String clientId) {
        ...
        List<BillingDtos.LedgerEntryDto> entryDtos = entries.stream().map(e -> BillingDtos.LedgerEntryDto.builder()
                .id(e.getId())
                .amount(e.getAmount())
                .type(e.getType().name())
