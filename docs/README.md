### **6. `docs/README.md`**
```markdown
# Documentation

Comprehensive documentation for the Logistics Platform.

## 📚 Table of Contents

### Architecture
- [System Overview](./architecture/system-overview.md)
- [Logistic Platform Architecture](./ARCHITECTURE_ANALYSIS.md) - **(Current) Unified Platform**
- [Database Design](./architecture/database-design.md)
- [API Design Guidelines](./architecture/api-design.md)
- [Security Architecture](./architecture/security.md)

### Development
- [Getting Started](./development/getting-started.md)
- [Development Environment Setup](./development/environment-setup.md)
- [Coding Standards](./development/coding-standards.md)
- [Testing Guidelines](./development/testing.md)
- [Debugging Guide](./development/debugging.md)

### API Documentation
- [API Overview](./api/overview.md)
- [Authentication](./api/authentication.md)
- [Tenant API](./api/tenant-api.md)
- [Order API](./api/order-api.md)
- [Parcel API](./api/parcel-api.md)
- [Webhook Documentation](./api/webhooks.md)

### Deployment & Setup
- [Infrastructure Knowledge Base](./infrastructure/README.md) - **Start here for service details.**
- [Local Setup Guide (No Docker)](./LOCAL_SETUP_GUIDE.md) - **Read this if running without Docker.**
- [Docker Operations Guide](./DOCKER_GUIDE.md) - **Read this for Docker-based setup.**
- [Database Setup](./DATABASE_SETUP_GUIDE.md)
- [General Deployment](./DEPLOYMENT_GUIDE.md)

### Operations
- [Monitoring Guide](./operations/monitoring.md)
- [Logging Guide](./operations/logging.md)
- [Performance Tuning](./operations/performance.md)
- [Disaster Recovery](./operations/disaster-recovery.md)
- [Backup Procedures](./operations/backup.md)

### Guides
- [Multi-tenancy Guide](./guides/multi-tenancy.md)
- [Payment Integration](./guides/payment-integration.md)
- [SMS/Email Integration](./guides/notifications.md)
- [Map Integration](./guides/map-integration.md)
- [Mobile App Integration](./guides/mobile-integration.md)

## 📖 How to Use This Documentation

### For Developers
1. Start with [Getting Started](./development/getting-started.md)
2. Read [Coding Standards](./development/coding-standards.md)
3. Refer to API documentation for integration

### For DevOps
1. Check [Deployment Guides](./deployment/)
2. Review [Monitoring Setup](./operations/monitoring.md)
3. Follow [Backup Procedures](./operations/backup.md)

### For API Consumers
1. Read [API Overview](./api/overview.md)
2. Check [Authentication Guide](./api/authentication.md)
3. Refer to specific service APIs

## 🔍 Quick Links
- [API Reference](https://api.logistics-platform.com)
- [Admin Dashboard](https://admin.logistics-platform.com)
- [Monitoring Dashboard](https://grafana.logistics-platform.com)
- [Error Tracking](https://sentry.logistics-platform.com)

## 📝 Contributing to Documentation
[Content continues...]